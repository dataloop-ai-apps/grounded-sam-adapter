...
class AsyncVideoFrameLoader:
    ...
    def __getitem__(self, index):
        ...
        img, video_height, video_width = self._load_img_as_tensor(
            img_pil=Image.fromarray(frame), image_size=self.image_size
        )
...
class Runner(dl.BaseServiceRunner):
    def __init__(self, dl):
        ...
        model_cfg = os.path.join(os.path.dirname(__file__), "sam2_hiera_s.yaml")
    ...
    def get_sam_features(self, dl, item):
        ...
        with open(f"tmp/{item.id}.json", "w") as f:
            json.dump(feeds, f)
        features_item = item.dataset.items.upload(
    ...
    def init_state(self, cap, image_size, num_frames, offload_video_to_cpu=False, offload_state_to_cpu=False):
        ...
        # Add missing key for frames tracked per object
    ...
    def track_polygons(self, dl, item_stream_url, bbs, start_frame, frame_duration=60, progress=None) -> dict:
        ...
        orig_item_builder = orig_item.annotations.builder()
            ...
            for out_frame_idx, out_obj_ids, out_mask_logits in self.video_predictor.propagate_in_video(
                ...
                for i, bbox_id in enumerate(out_obj_ids):
                    ...
                    if mask.any():
                        ...
                        # Convert UUID to numeric object ID for Dataloop compatibility
                        numeric_object_id = hash(bbox_id) % (2**31)  # Convert to positive 32-bit integer
                        orig_item_builder.add(
                            annotation_definition=video_segments[bbox_id][start_frame + out_frame_idx],
                            object_id=numeric_object_id,
                            frame_num=start_frame + out_frame_idx,
                        )
                    else:
    ...
    def box_to_segmentation(self, dl, item: dl.Item, annotations, progress: dl.Progress = None) -> list:
        return self.sam_predict_box(
            dl=dl, item=item, annotations=annotations, return_type="Semantic", progress=progress
        )

    def box_to_polygon(self, dl, item: dl.Item, annotations, progress: dl.Progress = None) -> list:
        return self.sam_predict_box(dl=dl, item=item, annotations=annotations, return_type="Polygon", progress=progress)

    def sam_predict_box(
        self, dl, item: dl.Item, annotations, return_type: str = "segment", progress: dl.Progress = None
    ) -> list:
        """
...
if __name__ == "__main__":
    runner = Runner(dl=dl)
    item = dl.items.get(item_id="67f67815baf34cdf8705487d")
    # points = [{"x": 1896, "y": 1242, "in": True}]
    bb = {
        "587e481b-0e20-4670-a67b-c276ef603936": [{"x": 597, "y": 359}, {"x": 1535, "y": 1008}]
    }
    color = [255, 0, 0]
    runner.track_polygons(
        dl=dl,
        item_stream_url="https://gate.dataloop.ai/api/v1/items/67f67815baf34cdf8705487d/stream",
        bbs=bb,
        start_frame=0,
        frame_duration=60
    )
